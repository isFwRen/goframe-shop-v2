...
type PositionReq struct {
	g.Meta    `path:"/backend/position/add" method:"post" tags:"手工位图" summary:"手工位图列表接口"`
...
type PositionUpdateReq struct {
	g.Meta `path:"/backend/position/update" method:"post" tags:"手工位图" summary:"手工位图列表接口"`
	Id     uint   `json:"id" v:"min:1#请选择要修改的手工位图" dc:"手工位图id"`
	PicUrl string `json:"pic_url" v:"required#图片链接不能为空" dc:"图片链接"`
	Link   string `json:"link" v:"required#跳转链接不能为空" dc:"跳转链接"`
	Sort   int    `json:"sort" dc:"排序"`
}
