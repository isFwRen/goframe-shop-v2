...
type RotationReq struct {
	g.Meta `path:"/rotation/add" method:"post" tags:"轮播图" summary:"添加轮播图"`
...
type RotationUpdateReq struct {
	g.Meta `path:"/rotation/update" method:"post" tags:"轮播图" summary:"修改轮播图"`
	Id     uint   `json:"id" v:"min:1#请选择要修改的轮播图" dc:"轮播图id"`
	PicUrl string `json:"pic_url" v:"required#图片链接不能为空" dc:"图片链接"`
	Link   string `json:"link" v:"required#跳转链接不能为空" dc:"跳转链接"`
	Sort   int    `json:"sort" dc:"排序"`
}
...
type RotationGetListCommonReq struct {
	g.Meta `path:"/rotation/list" method:"get" tags:"轮播图" summary:"获取轮播图"`
